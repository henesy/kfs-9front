	...
	union
	{
		struct
		{
			...
			Qid9p1	qid;		/* R-Attach, R-Clwalk, R-Walk,
						 * R-Open, R-Create */
			char	rauth[MAXAUTHENTLEN];	/* R-attach */
		};
		struct
		{
			char	uname[MAXNAMELEN];	/* T-nAttach */
			char	aname[MAXNAMELEN];	/* T-nAttach */
			char	ticket[MAXTICKETLEN];	/* T-attach */
			char	auth[MAXAUTHENTLEN];	/* T-attach */
		};
		struct
		{
			char	ename[ERRREC];	/* R-nError */
			char	chal[CHALLEN];	/* T-session, R-session */
			char	authid[MAXNAMELEN];	/* R-session */
			char	authdom[DOMLEN];	/* R-session */
		};
		struct
		{
			char	name[MAXNAMELEN];	/* T-Walk, T-Clwalk, T-Create, T-Remove */
			long	perm;		/* T-Create */
